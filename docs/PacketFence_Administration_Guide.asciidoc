...
First for debian, you must enable non-free and backports repository:
...
For Ubuntu you need to enable another repository:
...
Since PacketFence v5 you are now able to create multiples scan engines configuration and assign them on specific captive portals.
It mean per example that you are anow able to active a scan for specific Operating System only on a specific SSID.
>>>>>>> Updated scanner documentation
=======
In /usr/local/pf/raddb/sites-enabled/packetfence, modify the authorize section like this:
